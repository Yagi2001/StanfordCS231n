from builtins import range
import numpy as np
from random import shuffle
from past.builtins import xrange


def softmax_loss_naive(W, X, y, reg):
    """
    Softmax loss function, naive implementation (with loops)

    Inputs have dimension D, there are C classes, and we operate on minibatches
    of N examples.

    Inputs:
    - W: A numpy array of shape (D, C) containing weights.
    - X: A numpy array of shape (N, D) containing a minibatch of data.
    - y: A numpy array of shape (N,) containing training labels; y[i] = c means
      that X[i] has label c, where 0 <= c < C.
    - reg: (float) regularization strength

    Returns a tuple of:
    - loss as single float
    - gradient with respect to weights W; an array of same shape as W
    """
    # Initialize the loss and gradient to zero.
    loss = 0.0
    dW = np.zeros_like(W)


    #############################################################################
    # TODO: Compute the softmax loss and its gradient using explicit loops.     #
    # Store the loss in loss and the gradient in dW. If you are not careful     #
    # here, it is easy to run into numeric instability. Don't forget the        #
    # regularization!                                                           #
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    num_classes = W.shape[1]
    num_train = X.shape[0]
    for i in range(num_train):
        scores = X[i].dot(W)
        exp_of_scores = np.exp(scores)
        sum_of_scores = np.sum(exp_of_scores)
        normalized_scores = exp_of_scores/sum_of_scores
        correct_class_score = normalized_scores[y[i]]
        max_score = np.max(normalized_scores)
        C = np.exp(-max_score)    ## log C is equal to -maxscore
        loss+=C*-np.log(correct_class_score)/C
        for j in range(num_classes):
            if j == y[i]:
                dW[:,j]+=(normalized_scores[j]-1)*X[i]
            else:
                dW[:,j]+=normalized_scores[j]*X[i]

    loss/=num_train
    loss+= reg * np.sum(W * W)
    dW = dW/num_train
    dW += 2*reg*W


    pass

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****

    return loss, dW


def softmax_loss_vectorized(W, X, y, reg):
    """
    Softmax loss function, vectorized version.

    Inputs and outputs are the same as softmax_loss_naive.
    """
    # Initialize the loss and gradient to zero.
    loss = 0.0
    dW = np.zeros_like(W)

    #############################################################################
    # TODO: Compute the softmax loss and its gradient using no explicit loops.  #
    # Store the loss in loss and the gradient in dW. If you are not careful     #
    # here, it is easy to run into numeric instability. Don't forget the        #
    # regularization!                                                           #
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    num_classes = W.shape[1]
    num_train = X.shape[0]
    scores = X.dot(W) ##Scores without normalization
    scores = np.exp(scores)##Scores exponential
    sum_of_scores = np.sum(scores,axis=1)
    normalized_scores = scores/sum_of_scores[:,None]
    correct_class_scores = normalized_scores[range(scores.shape[0]), y]
    C = np.max(correct_class_scores)
    loss_vector = C*np.log(correct_class_scores)/-C
    loss+=np.sum(loss_vector)
    loss/= num_train
    loss+= reg*np.sum(W*W)

    gradient_vector = normalized_scores

    gradient_vector[range(scores.shape[0]),y] -=1 ## We add -1 for true class. Now we are ready to scale our vector with X
    dW = X.transpose().dot(gradient_vector)
    dW = dW/num_train
    dW += 2 * reg * W




    pass

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****

    return loss, dW
